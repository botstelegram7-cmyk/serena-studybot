...
async def start_mock_test(client: Client, message: Message,
    ...
    # Always force-clear any existing session before starting new one
...
async def process_button_answer(client, uid: int, chat_id: int,
    ...
    if next_idx >= total:
        await asyncio.sleep(2)   # Brief pause before analysis
        await finish_test(client, chat_id, uid)
    else:
        await asyncio.sleep(1.5)  # Anti-flood delay between questions
